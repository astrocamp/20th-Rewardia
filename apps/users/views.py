...
def prepare_card_form_context(
    ...
    # 取得所有活躍的卡片
    cards = CreditCard.objects.filter(is_active=True).only(
        "id", "name", "bank", "image"
    )
...
def member_zone(request):
    ...
    if request.user.is_authenticated:
        ...
        user_cards = request.user.user_cards.select_related("card").filter(
        ...
        all_rewards = (
            RewardCategory.objects.filter(card_id__in=card_ids, is_active=True)
            .annotate(
        ...
        for user_card in user_cards:
            ...
            # 處理卡號
            masked_card_number = (
                user_card.get_masked_card_number()
                if user_card.card_number_encrypted
                else "尚未登記卡號"
            )
            ...
            for reward in card_rewards:
                ...
                if (
                    category not in category_highest
                    or reward.max_effective_rate
                    > category_highest[category].max_effective_rate
                ):
                    category_highest[category] = reward

            # 按費率排序，取前三高
            top_rewards = sorted(
                category_highest.values(),
                key=lambda x: x.max_effective_rate,
                reverse=True,
            )[:3]
...
def get_cards_by_bank_htmx(request):
    ...
    if not bank_name:
        return render(
            request,
            "users/partials/card_options.html",
            {"cards": [], "message": "請選擇發卡銀行"},
        )
    ...
    try:
        ...
        return render(
            request,
            "users/partials/card_options.html",
            {"cards": cards, "bank_name": bank_name},
        )
    ...
    if request.method == "GET":
        ...
        if card_number and card_number.strip():
            ...
            if re.search(r"[^\d]", card_number_no_spaces):
                messages.error(
                    request, "卡號格式異常：只能包含數字，不允許字母或特殊符號"
                )
                context = prepare_card_form_context(
                    user_card=user_card,
                    is_edit_mode=is_edit_mode,
...
def card_add_number(request, card_id):
    ...
    if request.method == "POST":
        try:
            ...
            if not card_number:
                return JsonResponse({"success": False, "message": "卡號不能為空"})
            ...
            try:
                ...
                if decrypted and decrypted.strip():
                    return JsonResponse(
                        {"success": False, "message": "此卡片已有卡號，無法重複新增"}
                    )
            ...
            if len(clean_number) < 12 or len(clean_number) > 19:
                return JsonResponse(
                    {"success": False, "message": "卡號長度必須在12-19位之間"}
                )
            ...
            return JsonResponse(
                {"success": True, "message": f"成功為 {card_name} 新增卡號！"}
            )
            ...
            return JsonResponse(
                {"success": False, "message": "資料處理失敗，請檢查輸入格式"}
            )
        except Exception as e:
            # 未預期的系統錯誤
            logger = logging.getLogger(__name__)
            logger.error(f"Unexpected error in card_add_number: {e}")
            return JsonResponse({"success": False, "message": "系統錯誤，請稍後再試"})
...
def card_delete(request, card_id):
    ...
    if request.method == "POST":
        ...
        card_name = f"{user_card.card.bank} {user_card.card.name}"
        user_card.delete()
        return JsonResponse({"success": True, "message": f"成功刪除 {card_name}！"})
...
def change_password(request):
    ...
    if request.method != "POST":
        return JsonResponse(
            {"success": False, "message": "只允許 POST 請求"}, status=405
        )
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "message": "無效的 JSON 資料"}, status=400
        )
    except Exception as e:
        return JsonResponse(
            {"success": False, "message": f"修改密碼失敗：{str(e)}"}, status=500
        )
...
def identify_bank_by_bin(request):
    ...
    try:
        ...
        if not bin_code:
            return JsonResponse(
                {"success": False, "message": "BIN 碼不能為空"}, status=400
            )

        # 驗證 BIN 碼格式
        if len(bin_code) != 6 or not bin_code.isdigit():
            return JsonResponse(
                {"success": False, "message": "BIN 碼必須是6位數字"}, status=400
            )
        ...
        return JsonResponse(
            {"success": False, "message": "銀行辨識服務暫時無法使用"}, status=500
        )
