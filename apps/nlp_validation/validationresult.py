...
def validation_processors():
    """改寫版本 - 儲存到 PendingReward 模型"""
    config = load_config()
    ...
    for index, test_text in enumerate(test_cases, 1):
        ...
        try:
            ...
            if bank_name and card_name:
                ...
                if created:
                    print(f"建立新信用卡：{bank_name} {card_name}")

            if not credit_card:
                print(f"無法提取銀行或卡片名稱，跳過：{test_text[:50]}...")
                continue
    ...
    print(f"總測試時間: {time.strftime('%H:%M:%S', time.gmtime(total_time))}")
    print(f"平均時間 {(total_time / len(test_cases)):.3f} 秒/張")
    print("所有結果已儲存到 PendingReward 模型")
