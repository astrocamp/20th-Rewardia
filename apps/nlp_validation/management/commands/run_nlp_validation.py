    ...
    def handle(self, *args, **options):
        ...
        for index, test_text in enumerate(test_cases, 1):
            ...
            try:
                ...
                if not credit_card:
                    self.stdout.write(f"無法提取銀行或卡片名稱，跳過：{test_text[:50]}...")
                    continue
                ...
                self.stdout.write(f"=========={index} Done! ====<-{(time.time() - index_start_time):.3f}->====")
        ...
        self.stdout.write(f"總測試時間: {time.strftime('%H:%M:%S', time.gmtime(total_time))}")
