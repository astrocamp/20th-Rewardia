...
FAKE_SCOPES = {
    ...
    'CVS': {
        '7-11': {'min_rate': 2.0},
        '全家': {'min_rate': 1.5, 'max_rate': 3.0}
    },
    'GAS': {
        '中油': {'min_rate': 4.0, 'max_rate': 7.0},
        '台塑': {'min_rate': 3.5}
    }
}
...
def calculate_reward(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # 使用假資料計算回饋
            if category_code in FAKE_SCOPES and scope_name in FAKE_SCOPES[category_code]:
                ...
                if max_rate:
                    ...
                    messages.success(request, f'計算成功！{card.bank.name} {card.name} 在 {scope_name} 的回饋為 {result}')
                else:
                ...
                messages.warning(request, f'找不到 {card.bank.name} {card.name} 在 {scope_name} 的回饋資訊')
                return HttpResponse('''
